...
def fetch_legado(conn: pyodbc.Connection, ids: list[str], tabla_legado: str, batch_size: int = 500) -> pd.DataFrame:
    resultados = []
    cols_legado = [
        "ID_NUM_CONV",
        "ID_NUM_VER",
...
def fetch_kona(conn: pyodbc.Connection, faltantes: list[str]) -> pd.DataFrame:
    if not faltantes:
        print("Filas recuperadas desde SAP_KONA: 0")
        return pd.DataFrame()
...
def run_merge(job: MergeJob) -> str:
    print(f"\n=== Ejecutando merge: {job.name} ===")
    df_excel = load_excel(job.excel_path)
    values = unique_ids(df_excel, "Id")

    conn = connect_sql_server()
    try:
        df_sql_legado = fetch_legado(conn, values, job.tabla_legado, batch_size=500)
        ...
        output_path = os.path.join("output", job.output_filename)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ejecuta merges de CityClub/Soriana (07/08) desde un solo archivo.")
    parser.add_argument(
        "--job",
        choices=[*MERGE_JOBS.keys(), "all"],
        nargs="+",
        default=["all"],
        help="Que merge ejecutar (p. ej. cityclub07). Usa 'all' para correr todos.",
    )
